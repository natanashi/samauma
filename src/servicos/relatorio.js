/* SAMAÚMA — relatórios.
   ...
   O PDF sai da impressão do navegador: sem biblioteca, sem servidor e sem
   depender de fonte externa. O documento é montado num iframe e impresso ali,
   para não esbarrar em bloqueador de pop-up. */

const Relatorio = {

  /* ------------------------------------------------------------- escopos */

  /* Cada perfil exporta o seu recorte — não só uma demanda por vez. */
  escopo(nome, opcoes = {}) {
    switch (nome) {
      case 'gerador': {
  ...
  COLUNAS: [
    ['Demanda', d => d.id],
    ['Situação', d => STATUS[d.status].rotulo],
    ['Gerador', d => d.gerador.nome],
    ['CNPJ', d => { const g = Catalogo.gerador(d.gerador.id); return g ? g.cnpj : ''; }],
    ['Resíduo', d => Catalogo.nomeResiduo(d.residuo)],
    ['Ponto de coleta', d => d.ponto],
    ['Bairro', d => d.bairro],
    ['Zona', d => d.zona],
    ['Estimado (kg)', d => d.estimadoKg],
    ['Coletado (kg)', d => d.coletadoKg],
    ['Recebido (kg)', d => d.verificadoKg],
    ['Reciclado (kg)', d => Demanda.reciclado(d)],
    ['Rejeito (kg)', d => Demanda.rejeito(d)],
    ['Recuperação (%)', d => Demanda.taxaRecuperacao(d)],
    ['Divergência (%)', d => Demanda.divergencia(d)],
    ['Catador', d => d.catador ? d.catador.nome : ''],
    ['Vínculo', d => {
      if (!d.catador) return '';
      const coop = Catalogo.cooperativa(d.catador.cooperativa);
      return coop ? coop.nome : 'autônomo';
    }],
    ['Destinatário', d => d.destino ? d.destino.nome : ''],
    ['Destino do material', d => d.status === 'COMPROVADA' ? Demanda.destinoFinal(d) : ''],
    ['Lote', d => d.lote || ''],
    ['Valor (R$)', d => Demanda.valor(d)],
    ['CO2e evitado (kg)', d => Demanda.co2(d)],
    ['Criada em', d => Fmt.dataHora(d.criadaEm)],
    ['Prazo', d => Fmt.data(d.prazo)],
    ['Recebida em', d => d.recebidaEm ? Fmt.dataHora(d.recebidaEm) : ''],
    ['Comprovante', d => d.comprovante ? d.comprovante.codigo : ''],
    ['Emitido em', d => d.comprovante ? Fmt.dataHora(d.comprovante.emitidoEm) : '']
  ],

  /* Ponto e vírgula como separador e vírgula decimal: é o que a planilha em
     português espera. O BOM faz o Excel abrir em UTF-8 sem estragar acento. */
  csv(escopo) {
    const celula = valor => {
  ...
  /* O mesmo recorte, agora legível: cabeçalho, resumo, composição e a tabela
     dos processos. Sai pela impressão do navegador — "Salvar como PDF". */
  documentoHtml(escopo) {
    const linha = (rotulo, valor) => `<tr><th>${escapar(rotulo)}</th><td>${escapar(valor)}</td></tr>`;
...
<footer class="folha-pe">
  ...
  /* Imprime num iframe escondido: o navegador oferece "Salvar como PDF". */
  imprimir(nomeEscopo, opcoes) {
    const escopo = Relatorio.escopo(nomeEscopo, opcoes);
    ...
    doc.open();
    doc.write(Relatorio.documentoHtml(escopo));
    doc.close();
...
/* Estilo do documento impresso: papel A4, sem cor de fundo pesada e com a
   assinatura de três traços da marca. */
