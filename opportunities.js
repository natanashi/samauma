...
const oppAccepted=[];
const isCollectorOpportunityView=()=>typeof currentSystemProfile!=='undefined'&&currentSystemProfile==='catador';
...
function getVisibleOpportunities(){
  ...
  const collector=isCollectorOpportunityView();
  return OPP_POINTS.filter(p=>(!collector||p.released)&&(material==='all'||p.material===material||material==='secos'&&['secos','papelao','plastico'].includes(p.material))&&p.km<=distance&&(period==='all'||p.period===period)&&!oppAccepted.some(a=>a.id===p.id&&a.actor===(collector?'catador':'cooperativa')));
}

function applyOpportunityProfileCopy(){
  const collector=isCollectorOpportunityView();
  const copy=collector?{
    eyebrow:'TRABALHOS LIBERADOS PELA COOPERATIVA · PORTO VELHO',title:'Escolha entre trabalhos que a cooperativa já autorizou',description:'Consulte distância, material, horário e valor previsto antes de confirmar sua disponibilidade. A contratação e a capacidade já foram avaliadas pela organização.',ruleLabel:'REGRA DE GOVERNANÇA',ruleTitle:'A cooperativa aceita. Você confirma disponibilidade.',ruleDescription:'O catador não assume contrato comercial. Ele escolhe entre trabalhos liberados ou responde a uma atribuição.',listTab:'Trabalhos',routesTab:'Minhas rotas',listLabel:'TRABALHOS DISPONÍVEIS PARA A EQUIPE',routesEyebrow:'TRABALHO JÁ ORGANIZADO',routesTitle:'Minhas rotas',routesDescription:'Trabalhos que você confirmou e rotas atribuídas pela cooperativa.',notice:'Todos os trabalhos desta tela já foram aceitos e liberados pela cooperativa. Dados demonstrativos.'
  }:{
    eyebrow:'DEMANDAS COMERCIAIS · PORTO VELHO',title:'A cooperativa avalia a demanda antes de assumir o serviço',description:'Consulte material, distância, remuneração, capacidade e janela antes de aceitar uma demanda comercial.',ruleLabel:'REGRA DE GOVERNANÇA',ruleTitle:'O sistema recomenda. A cooperativa decide.',ruleDescription:'Somente depois do aceite a organização libera o trabalho ou atribui uma equipe.',listTab:'Demandas',routesTab:'Operações aceitas',listLabel:'DEMANDAS COMERCIAIS DISPONÍVEIS',routesEyebrow:'OPERAÇÃO JÁ ORGANIZADA',routesTitle:'Operações aceitas',routesDescription:'Demandas assumidas pela cooperativa e prontas para organização da equipe.',notice:'Valores, locais e organizações são fictícios. A geografia usa referências reais de Porto Velho.'
  };
  const values={oppHeroEyebrow:copy.eyebrow,oppHeroTitle:copy.title,oppHeroDescription:copy.description,oppRuleLabel:copy.ruleLabel,oppRuleTitle:copy.ruleTitle,oppRuleDescription:copy.ruleDescription,oppListTabLabel:copy.listTab,oppRoutesTabLabel:copy.routesTab,oppListSectionLabel:copy.listLabel,oppRoutesEyebrow:copy.routesEyebrow,oppRoutesTitle:copy.routesTitle,oppRoutesDescription:copy.routesDescription,oppDataNotice:copy.notice};
  Object.entries(values).forEach(([id,value])=>{const el=document.getElementById(id);if(el)el.textContent=value;});
  const legend=document.getElementById('oppRouteLegend');if(legend)legend.innerHTML=collector?'<i class="chosen">Disponibilidade confirmada</i><i class="assigned">Atribuída pela cooperativa</i>':'<i class="chosen">Aceita pela cooperativa</i><i class="assigned">Equipe atribuída</i>';
}
...
function opportunityCard(p){
  const collector=isCollectorOpportunityView(),recommended=p.best?collector?'RECOMENDADO PARA SUA EQUIPE':'RECOMENDADA PARA A COOPERATIVA':'';
  return `<article class="opportunity-card ${p.best?'recommended':''}">
    <div class="opportunity-card-top"><div>${p.best?`<span class="opportunity-card-tag">${recommended}</span>`:`<span class="data-origin declared">${collector?'LIBERADO PELA COOPERATIVA':'DEMANDA COMERCIAL'}</span>`}</div><span class="opportunity-card-code">${p.id}</span></div>
    ...
    <div class="opportunity-card-actions"><button class="btn sec" onclick="openOpportunityDetail('${p.id}')">Ver detalhes</button><button class="btn" onclick="beginOpportunityAcceptance('${p.id}')">${collector?'Tenho disponibilidade':'Aceitar demanda'}</button><span class="reason"><b>${p.score}% compatível</b>${p.best?'Melhor combinação agora':'Cálculo transparente'}</span></div>
  </article>`;
}

function renderOpportunityChooser(){
  applyOpportunityProfileCopy();
  ...
  const collector=isCollectorOpportunityView(),noun=collector?'trabalho':'demanda';
  const title=document.getElementById('oppResultTitle');if(title)title.textContent=`${visible.length} ${visible.length===1?noun+' encontrado':noun+'s encontrados'}`;
...
function opportunityDetailMarkup(p){
  const collector=isCollectorOpportunityView();
  return `<div class="opportunity-detail-head"><button aria-label="Fechar detalhes" onclick="closeOpportunityDetail()">×</button><span>${p.best?(collector?'RECOMENDADO PARA SUA EQUIPE':'RECOMENDADA PARA A COOPERATIVA'):(collector?'TRABALHO LIBERADO':'DEMANDA COMERCIAL')} · ${p.score}%</span><h2>${p.name}</h2><p>${p.bairro} · dados demonstrativos</p></div><div class="opportunity-detail-body">
    <div class="opportunity-detail-status"><b>${collector?'A contratação já foi avaliada pela cooperativa':'A cooperativa decide se assume a demanda'}</b><p>${collector?'Sua confirmação informa disponibilidade para executar; não cria contrato com o gerador.':'A recomendação não aceita nem reserva a demanda automaticamente.'}</p></div>
    ...
    <p class="opportunity-accept-note"><b>${collector?'Ao confirmar disponibilidade:':'Ao aceitar a demanda:'}</b> ${collector?'o trabalho entra em “Minhas rotas” e a cooperativa recebe sua confirmação.':'a operação fica reservada para a cooperativa organizar veículo, equipe e rota.'}</p>
  </div><div class="opportunity-detail-actions"><button class="btn sec" onclick="closeOpportunityDetail()">Agora não</button><button class="btn" onclick="beginOpportunityAcceptance('${p.id}')">${collector?'Tenho disponibilidade':'Aceitar demanda'}</button></div>`;
}
...
function beginOpportunityAcceptance(id){
  ...
  const collector=isCollectorOpportunityView();
  drawer.hidden=false;drawer.innerHTML=`<div class="opportunity-detail-head"><button aria-label="Fechar confirmação" onclick="closeOpportunityDetail()">×</button><span>${collector?'CONFIRMAÇÃO DE DISPONIBILIDADE':'ACEITE DA COOPERATIVA'}</span><h2>${p.name}</h2><p>${p.bairro} · ${p.window}</p></div><div class="reservation-card"><i>✓</i><h2>${collector?'Você está disponível para este trabalho?':'A cooperativa deseja assumir esta demanda?'}</h2><p>${collector?'A cooperativa já avaliou a demanda. Sua confirmação inclui o trabalho na sua rota e informa a coordenação.':'O aceite reserva a demanda para a organização definir equipe, veículo e percurso. Nenhum catador será escalado automaticamente.'}</p><span class="reservation-clock">${collector?'TRABALHO LIBERADO · CONFIRMAÇÃO PESSOAL':'RESERVA OPERACIONAL · 10:00'}</span><div class="opportunity-detail-grid"><div><span>SERVIÇO</span><b>R$ ${p.pay}</b></div><div><span>MASSA ESTIMADA</span><b>${p.kg} kg</b></div><div><span>DISTÂNCIA</span><b>${String(p.km).replace('.',',')} km</b></div><div><span>JANELA</span><b>${p.window}</b></div></div></div><div class="opportunity-detail-actions"><button class="btn sec" onclick="openOpportunityDetail('${p.id}')">Revisar detalhes</button><button class="btn" onclick="confirmOpportunityAcceptance('${p.id}')">${collector?'Confirmar disponibilidade':'Confirmar aceite da cooperativa'}</button></div>`;drawer.querySelector('button')?.focus();
}

function confirmOpportunityAcceptance(id){
  const p=OPP_POINTS.find(item=>item.id===id),collector=isCollectorOpportunityView(),actor=collector?'catador':'cooperativa';if(!p||oppAccepted.some(item=>item.id===id&&item.actor===actor))return;
  oppAccepted.push({...p,actor,origin:collector?'confirmed':'cooperative',route:collector?`Rota confirmada ${String(oppAccepted.filter(a=>a.actor==='catador').length+1).padStart(2,'0')}`:`Operação ${String(oppAccepted.filter(a=>a.actor==='cooperativa').length+1).padStart(2,'0')}`,status:collector?'Disponibilidade confirmada':'Aceita pela cooperativa'});closeOpportunityDetail();renderOpportunityChooser();setOppView('routes');toast(collector?'Disponibilidade confirmada · trabalho incluído em Minhas rotas':'Demanda aceita · pronta para organizar equipe e veículo');
}

function renderMyRoutes(){
  const target=document.getElementById('oppMyRoutes');if(!target)return;const collector=isCollectorOpportunityView(),routes=collector?[OPP_ASSIGNED_ROUTE,...oppAccepted.filter(a=>a.actor==='catador')]:oppAccepted.filter(a=>a.actor==='cooperativa');
  const count=document.getElementById('oppRouteCount');if(count)count.textContent=routes.length;
  if(!routes.length){target.innerHTML=`<div class="empty" style="grid-column:1/-1"><div class="ei">◇</div>Nenhuma demanda foi aceita pela cooperativa nesta simulação.<br>Volte à lista para avaliar capacidade, preço e janela.</div>`;return;}
  target.innerHTML=routes.map(r=>{
    const assigned=r.origin==='assigned',cooperative=r.origin==='cooperative',originLabel=assigned?'ATRIBUÍDA PELA COOPERATIVA':cooperative?'ACEITA PELA COOPERATIVA':'DISPONIBILIDADE CONFIRMADA';
    return `<article class="my-route-card ${assigned?'assigned':''}"><header><span class="route-origin ${assigned?'assigned':'chosen'}">${originLabel}</span><span class="st s-reg np">${r.status}</span></header><h3>${r.name}</h3><p>${r.bairro} · ${r.materialLabel}</p><div class="my-route-stats"><div><span>MASSA</span><b>${r.kg} kg</b></div><div><span>SERVIÇO</span><b>R$ ${r.pay}</b></div><div><span>JANELA</span><b>${r.window}</b></div></div>${assigned?`<p><b>Origem:</b> ${r.assignedBy}. Você pode informar indisponibilidade antes de iniciar.</p>`:cooperative?'<p><b>Próxima ação:</b> designar equipe, veículo e responsável ou liberar o trabalho para manifestação da equipe.</p>':'<p><b>Confirmação:</b> sua disponibilidade foi informada à cooperativa. A coordenação continua responsável pela rota.</p>'}<div class="opportunity-card-actions" style="margin-top:10px">${cooperative?`<button class="btn" onclick="releaseCooperativeWork('${r.id}')">Liberar para a equipe</button><button class="btn sec" onclick="toast('Equipe atribuída no protótipo · origem registrada')">Atribuir equipe</button>`:`<button class="btn" onclick="startAcceptedRoute('${r.id}')">Abrir rota</button>${assigned?'<button class="btn sec" onclick="toast(\'Indisponibilidade registrada · a cooperativa será avisada\')">Não posso atender</button>':''}`}</div></article>`;
  }).join('');
}

function releaseCooperativeWork(id){const p=OPP_POINTS.find(item=>item.id===id);if(!p)return;p.released=true;toast('Trabalho liberado para manifestação da equipe · origem preservada');}

function prepareOpportunityForProfile(){oppCurrentView='list';oppSelectedId=null;closeOpportunityDetail();setOppView('list');}

function startAcceptedRoute(id){
  const selected=id===OPP_ASSIGNED_ROUTE.id?OPP_ASSIGNED_ROUTE:oppAccepted.find(item=>item.id===id&&item.actor==='catador');if(!selected)return;
